 ...
 */
function createAnonymousInlineBox(textContent: string, textNode?: Text): BoxNode {
  return {
    id: generateId(),
 ...
 */
function getBoxTypeFromDisplay(displayType: DisplayType, isListItem: boolean): BoxType {
  if (isListItem) {
    return 'list-item'
  }
...
function createBoxFromElement(
  ...
  if (parentBlockifies && parentDisplayType) {
    ...
    const parentIsGrid = parentDisplayType === 'grid' || parentDisplayType === 'inline-grid'
  ...
  const box: BoxNode = {
    ...
    children: [],
    isAnonymous: false,
    isEmpty: element.children.length === 0 && (!element.textContent || element.textContent.trim().length === 0),
  }
...
function hasBlockLevelChildren(element: Element): boolean {
  for (let i = 0; i < element.childNodes.length; i++) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      ...
      const childEstablishesBFC = establishesBlockFormattingContext(displayType)
  ...
  for (const node of childNodes) {
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      if (isContents(displayType)) {
        ...
        const contentsChildren = processChildren(element, inlineDisplayType, null)
      ...
      if (isBlock) {
        ...
        const blockBox = createBoxFromElement(element, displayType, parentBlockifies, null)
        blockBox.children = processChildren(element, displayType, blockBox.boxType)
        ...
        const childEstablishesBFC = establishesBlockFormattingContext(displayType)
        const childHasBlocks = !childEstablishesBFC && hasBlockLevelChildren(element)
        ...
        if (childHasBlocks) {
          ...
          const nestedSplitBoxes = processInlineWithBlockChildren(element, displayType, parentBlockifies)
          ...
          const inlineBox = createBoxFromElement(element, displayType, parentBlockifies, null)
          inlineBox.children = processChildren(element, displayType, inlineBox.boxType)
...
function processChildren(
  ...
  const parentBlockifies = parentDisplayType ? blockifiesChildren(parentDisplayType) : false
  const parentIsBlockLevel = parentDisplayType ? isBlockLevel(parentDisplayType) : true
  ...
  while (i < childNodes.length) {
    ...
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      // Create anonymous inline box for text
      ...
      if (isContents(displayType)) {
        const contentsChildren = processChildren(element, parentDisplayType, parentBoxType)
      ...
      const elementEstablishesBFC = establishesBlockFormattingContext(displayType)
      
      if (elementIsInlineLevel && !parentEstablishesBFC && hasBlockChildren && !elementEstablishesBFC) {
        ...
        const splitBoxes = processInlineWithBlockChildren(element, displayType, parentBlockifies)
      ...
      const box = createBoxFromElement(element, displayType, parentBlockifies, parentDisplayType)
  ...
  const isParentBlockContainer = parentDisplayType ? isBlockContainer(parentDisplayType) : false
  ...
  if (isParentBlockContainer && !parentBlockifies) {
    ...
    const hasBlockLevelChildren = children.some(child => {
      const isBlock = child.boxType === 'block' || 
...
function wrapInlineInAnonymousBlocks(children: BoxNode[]): BoxNode[] {
  ...
  for (const child of children) {
    const isInline = child.boxType === 'inline' || child.boxType === 'inline-block' || child.boxType === 'anonymous-inline'
    const isBlock = child.boxType === 'block' || child.boxType === 'flex-container' || child.boxType === 'grid-container' || child.boxType === 'list-item'
 ...
 */
export function generateBoxTree(html: string): { tree: BoxNode | null; elementMap: ElementToBoxMap } {
  nodeIdCounter = 0
  ...
  try {
    ...
    if (bodyElementChildren.length === 1 && !hasNonWhitespaceText) {
      ...
      const childEstablishesBFC = establishesBlockFormattingContext(displayType)
      const needsSplitting = isInline && !childEstablishesBFC && hasBlockLevelChildren(singleChild)
